...
### [TODO-004] 彻底消除“没有生成有效回复”冷提示与 AnyBot 优雅重启
- **任务编号**：`TODO-004`
- **登记时间**：2026-09-17 22:55
- **优先级**：高 (P1)
- **当前状态**：⏳ 等待审批 (WAITING_APPROVAL)
- **问题背景**：
  1. 老山爹在飞书对话时，若后台 `agy` 执行复杂任务耗时过长（如 09-16 22:40 排查喷嚏网耗时 304 秒），底层 CLI 会因 5 分钟硬超时退出，导致 `stdoutChars: 0`；
  2. 此前在另一终端中富贵曾向老山爹汇报过重构方案，但后续因转入探讨 `agy` 超时与压缩机制，该修改并未实际落地到代码；
  3. `src/index.ts`（第 142 行）仍硬编码返回“没有生成有效回复，请换个方式描述试试。”，且系统进程自 9 月 14 日以来从未重启过；
  4. 该冷提示未能告知老山爹后台到底发生了什么，体验极差。
- **实施方案**：
  1. **重构错误文案**：修改 `src/index.ts` 的 `formatProviderError`，将 `ProviderEmptyOutputError` 替换为透明、具备指导意义的详尽提示（明确说明后台模型或任务耗时超限未组织出回复，提示可使用 `/retry` 重新生成或简化提问）；
  2. **静态校验**：调用 `static-validator` 深度检查确保语法与类型 100% 通过；
  3. **优雅重启服务**：按照《僵尸进程避坑指南》，在 tmux `anybot` 中精准重启 AnyBot，确保新逻辑切实生效。
- **影响文件**：`/root/AnyBot-Dev/src/index.ts`
